...
class Container_of_plugins(Plugin, InSpeed, OutSpeed):
    def __init__(self, *args, **kwargs):
        ...
        self.generator_height = Generator(plugin_name='HeightGenerator', expression=lambda loop_index: math.cos(loop_index / 100) * 100, csv_out='./3st_out.csv_testdata' )
        self.mover = Mover(plugin_name='Mover', csv_out='./4st_out.csv_testdata')

        # Connect nodes
        self.smoother = Smoother(plugin_name='SpeedSmoother', in_window_size=20, speed_change_limit=20, csv_out='./1st_out.csv_testdata')
